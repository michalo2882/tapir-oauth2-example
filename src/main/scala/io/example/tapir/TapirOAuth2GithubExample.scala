...
object TapirOAuth2GithubExample extends App {
  ...
  val jwtKey = "..." // generate long hex string
  ...
  def authenticate(token: String): IO[Either[String, String]] = {
    IO(JwtCirce.decodeAll(token, jwtKey, Seq(jwtAlgo)).toEither
  ...
  val secretPlaceRoute: HttpRoutes[IO] = Http4sServerInterpreter.toRoutes(secretPlace)(
    token =>
      (for {
        authDetails <- EitherT(authenticate(token))
        msg <- EitherT.fromEither(("Your details: " + authDetails).asRight[String])
      } yield msg).value
  )
